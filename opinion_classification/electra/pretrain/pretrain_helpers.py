...
def scatter_update(sequence, updates, positions):
    ...
    if sequence.dtype == tf.float32:
        ...
        updates = tf.floordiv(updates, tf.maximum(1, updates_mask_3d))
...
def _get_candidates_mask(inputs: pretrain_data.Inputs, vocab, disallow_from_mask=None):
...
def mask(
    ...
    vocab = tokenization.FullTokenizer(config.vocab_file, do_lower_case=config.do_lower_case).vocab
    ...
    masked_lm_positions = tf.multinomial(sample_logits, N)
    ...
    replace_with_mask_positions = masked_lm_positions * tf.cast(tf.less(tf.random_uniform([B, N]), 0.85), tf.int32)
...
def sample_from_softmax(logits, disallow=None):
    ...
    uniform_noise = tf.random_uniform(modeling.get_shape_list(logits), minval=0, maxval=1)

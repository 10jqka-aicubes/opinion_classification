...
def get_input_fn(config: configure_pretraining.PretrainingConfig, is_training, num_cpu_threads=4):
    ...
    def input_fn(params):
        ...
        d = d.apply(
            tf.contrib.data.parallel_interleave(tf.data.TFRecordDataset, sloppy=is_training, cycle_length=cycle_length)
        )

        ...
        if input_reprs is None:
            if input_embeddings is None:
                with tf.variable_scope(
                    (scope if untied_embeddings else "electra") + "/embeddings", reuse=tf.AUTO_REUSE
                ):
        ...
        with tf.variable_scope(scope, default_name="electra"):
            if self.embedding_output.shape[-1] != bert_config.hidden_size:
                self.embedding_output = tf.layers.dense(
                    self.embedding_output, bert_config.hidden_size, name="embeddings_project"
                )
            ...
            with tf.variable_scope("encoder"):
                ...
                self.pooled_average_output = tf.reduce_mean(self.sequence_output, axis=1)

    def get_pooled_output(self):
        return self.pooled_output

    def get_pooled_average_output(self):
        return self.pooled_average_output

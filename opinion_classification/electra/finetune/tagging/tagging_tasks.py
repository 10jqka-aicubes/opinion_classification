...
class TaggingExample(task.Example):
    ...
    def __init__(self, eid, task_name, words, tags, is_token_level, label_mapping):
        ...
        if is_token_level:
            ...
            labels = tagging_utils.get_tags(span_labels, len(words), LABEL_ENCODING)
...
class TaggingTask(task.Task):
    ...
    def __init__(self, config: configure_finetuning.FinetuningConfig, name, tokenizer, is_token_level):
        ...
        self._label_mapping_path = os.path.join(
            ...
            ("debug_" if self.config.debug else "") + self.name + "_label_mapping.pkl",
        )
    ...
    def _get_label_mapping(self, provided_split=None, provided_sentences=None):
        ...
        for split in ["train", "dev", "test"]:
            if not tf.io.gfile.exists(os.path.join(self.config.raw_data_dir(self.name), split + ".txt")):
                continue
        ...
        if self.name == "ccg":
            ...
            label_mapping = {
                label: i
                for i, label in enumerate(sorted(filter(lambda t: t not in infrequent_tags, tag_counts.keys())))
            }
            n = len(label_mapping)
    ...
    def _get_labeled_sentences(self, split):
        sentences = []
        with tf.io.gfile.GFile(os.path.join(self.config.raw_data_dir(self.name), split + ".txt"), "r") as f:
            sentence = []
            for line in f:
    ...
    def get_scorer(self):
        return (
            tagging_metrics.AccuracyScorer()
            if self._is_token_level
            else tagging_metrics.EntityLevelF1Scorer(self._get_label_mapping())
        )

    def get_feature_specs(self):
        return [
            feature_spec.FeatureSpec(self.name + "_eid", []),
            feature_spec.FeatureSpec(self.name + "_labels", [self.config.max_seq_length]),
            feature_spec.FeatureSpec(self.name + "_labels_mask", [self.config.max_seq_length], is_int_feature=False),
            feature_spec.FeatureSpec(self.name + "_labeled_positions", [self.config.max_seq_length]),
        ]
    ...
    def get_prediction_module(self, bert_model, features, is_training, percent_done):
        ...
        losses = tf.nn.softmax_cross_entropy_with_logits(
            labels=tf.one_hot(features[self.name + "_labels"], n_classes), logits=logits
        )

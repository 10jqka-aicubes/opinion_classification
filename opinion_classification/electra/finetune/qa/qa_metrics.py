...
class SpanBasedQAScorer(scorer.Scorer):
    ...
    def __init__(self, config: configure_finetuning.FinetuningConfig, task, split, v2):
    ...
    def _get_results(self):
        ...
        if self._name == "squad":
            ...
            return sorted(squad_official_eval_v1.main(self._config, self._split, self._name).items())
        else:
            return sorted(mrqa_official_eval.main(self._config, self._split, self._name).items())
    ...
    def write_predictions(self):
        ...
        for example in self._eval_examples:
            example_id = (
                example.qas_id
                if ("squad" in self._name or "cmrc2018" in self._name or "drcd" in self._name)
                else example.qid
            )
            ...
            for (feature_index, feature) in enumerate(features):
                ...
                if self._config.joint_prediction:
                    ...
                    start_indexes = _get_best_indexes(result.start_logits, self._config.n_best_size)
                    end_indexes = _get_best_indexes(result.end_logits, self._config.n_best_size)
                ...
                for i, start_index in enumerate(start_indexes):
                    for j, end_index in enumerate(end_indexes[i] if self._config.joint_prediction else end_indexes):
                        ...
                        if end_index not in feature[self._name + "_token_to_orig_map"]:
                            continue
                        if not feature[self._name + "_token_is_max_context"].get(start_index, False):
                            continue
            ...
            if self._v2:
                if len(prelim_predictions) == 0 and self._config.debug:
                    ...
                    prelim_predictions.append(
                        _PrelimPrediction(
                            feature_index=0, start_index=tokid, end_index=tokid + 1, start_logit=1.0, end_logit=1.0
                        )
                    )
            ...
            for pred in prelim_predictions:
                ...
                tok_tokens = feature[self._name + "_tokens"][pred.start_index : (pred.end_index + 1)]
                orig_doc_start = feature[self._name + "_token_to_orig_map"][pred.start_index]
                orig_doc_end = feature[self._name + "_token_to_orig_map"][pred.end_index]
        ...
        utils.write_json(dict(all_predictions), self._config.qa_preds_file(self._name + "_" + self._split))
        if self._v2:
            utils.write_json(
...
def get_final_text(config: configure_finetuning.FinetuningConfig, pred_text, orig_text):
    ...
    if len(orig_ns_text) != len(tok_ns_text):
        if config.debug:
            utils.log("Length not equal after stripping spaces: '%s' vs '%s'", orig_ns_text, tok_ns_text)
        return orig_text

...
class QATask(task.Task):
    ...
    def __init__(self, config: configure_finetuning.FinetuningConfig, name, tokenizer, v2=False):
    ...
    def featurize(self, example: QAExample, is_training, log=False, for_eval=False):
        all_features = []
        ...
        if is_training and not example.is_impossible:
            ...
            (tok_start_position, tok_end_position) = _improve_answer_span(
                all_doc_tokens, tok_start_position, tok_end_position, self._tokenizer, example.orig_answer_text
            )
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if log:
                ...
                utils.log("tokens: %s" % " ".join([tokenization.printable_text(x) for x in tokens]))
                utils.log(
                    "token_to_orig_map: %s"
    ...
    def get_prediction_module(self, bert_model, features, is_training, percent_done):
        ...
        end_top_log_probs = tf.zeros([batch_size, self.config.beam_size, self.config.beam_size])
        end_top_index = tf.zeros([batch_size, self.config.beam_size, self.config.beam_size], tf.int32)
        if self.config.joint_prediction:
            start_logits += 1000.0 * (answer_mask - 1)
            start_log_probs = tf.nn.log_softmax(start_logits)
            start_top_log_probs, start_top_index = tf.nn.top_k(start_log_probs, k=self.config.beam_size)

            if not is_training:
                # batch, beam, length, hidden
                end_features = tf.tile(tf.expand_dims(final_hidden, 1), [1, self.config.beam_size, 1, 1])
                # batch, beam, length
                start_index = tf.one_hot(start_top_index, depth=seq_length, axis=-1, dtype=tf.float32)
                # batch, beam, hidden
                start_features = tf.reduce_sum(
                    tf.expand_dims(final_hidden, 1) * tf.expand_dims(start_index, -1), axis=-2
                )
                # batch, beam, length, hidden
                start_features = tf.tile(tf.expand_dims(start_features, 2), [1, 1, seq_length, 1])
            else:
                start_index = tf.one_hot(
                    features[self.name + "_start_positions"], depth=seq_length, axis=-1, dtype=tf.float32
                )
                start_features = tf.reduce_sum(tf.expand_dims(start_index, -1) * final_hidden, axis=1)
                start_features = tf.tile(tf.expand_dims(start_features, 1), [1, seq_length, 1])
            ...
            end_logits = tf.squeeze(tf.layers.dense(final_repr, 1), -1, name="qa_logits")
            if is_training:
            ...
            if not is_training:
                ...
                end_top_log_probs, end_top_index = tf.nn.top_k(end_log_probs, k=self.config.beam_size)
        ...
        if self.config.answerable_classifier:
            ...
            if self.config.answerable_uses_start_logits:
                ...
                start_feature = tf.reduce_sum(tf.expand_dims(start_p, -1) * final_hidden, axis=1)
            ...
            answerable_loss = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.cast(features[self.name + "_is_impossible"], tf.float32), logits=answerable_logit
            )
...
class MRQATask(QATask):
    ...
    def get_examples(self, split):
        ...
        examples = []
        example_failures = [0]
        with tf.io.gfile.GFile(os.path.join(self.config.raw_data_dir(self.name), split + ".jsonl"), "r") as f:
        ...
        utils.log("{:} examples created, {:} failures".format(len(examples), example_failures[0]))
        return examples
...
class SQuADTask(QATask):
    ...
    def get_examples(self, split):
        ...
        with tf.io.gfile.GFile(
            os.path.join(
                self.config.raw_data_dir(self.name), split + ("-debug" if self.config.debug else "") + ".json"
            ),
            "r",
        ) as f:
